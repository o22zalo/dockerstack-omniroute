...
async function main() {
    ...
    try {
      ...
      if (dnsCfgRes.status === 200 && dnsCfgRes.body && typeof dnsCfgRes.body === "object") {
        ...
        if (candidate) {
          ...
          warnings.push(
            `dns/configuration did not include a usable tailnet search path (keys: ${topLevelKeys.join(", ") || "(none)"}).`,
          );
        }
  ...
  if (!apiTailnetDomain) {
    if (existingTailnetDomain && isLikelyTailnetDomain(existingTailnetDomain)) {
      ...
      warnings.push(
        `Could not infer TAILSCALE_TAILNET_DOMAIN from Tailscale API; using existing env value (unverified): ${existingTailnetDomain}.`,
      );
    } else {
      errors.push(
